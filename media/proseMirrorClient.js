...
`;
  ...
  // src/modules/proseMirrorEditorView/utils/textfileExtensions.ts
  var TextFileParser = class {
    parse(text2) {
      var nodes = [];
      try {
        text2.split("\n").forEach((line) => {
  ...
  var TextFileSerializer = class {
    serialize(content) {
      let result = "";
      content.forEach((node) => {
        result += node.textContent + node.isBlock ? "\n" : "";
      });
      return result;
    }
  };
  var textFileParser = new TextFileParser();
  var textFileSerializer = new TextFileSerializer();

  // src/modules/proseMirrorEditorView/utils/markdownExtensions.ts
  ...
  // src/modules/proseMirrorEditorView/utils/fileExtensions.ts
  var useMarkdown = true;
  var fileParser = useMarkdown ? defaultMarkdownParser : textFileParser;
  var fileSerializer = useMarkdown ? customMarkdownSerializer : textFileSerializer;

  // src/modules/proseMirrorEditorView/proseMirrorClient.ts
  var vscode = globalThis.acquireVsCodeApi();
  var previousState = vscode.getState()?.text || "";
  var doc3 = fileParser.parse(previousState);
  var typing = window.performance.now();
  var scrolbarrVisible = true;
  var originalScrolbarColor = document.querySelector("html").style.scrollbarColor;
  console.log(originalScrolbarColor);
  var hideScrollbarSubs = setInterval(() => {
    if (typing + 2e3 < window.performance.now()) {
      if (!scrolbarrVisible) {
  ...
  function updateContent(text2) {
    const doc4 = fileParser.parse(text2);
  ...
  document.querySelector("#toolbar").addEventListener("mouseenter", (e) => {
    const elem = document.querySelector(".ProseMirror-menubar");
...
//# sourceMappingURL=proseMirrorClient.js.map
